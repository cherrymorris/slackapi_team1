class GroupMessageController < ApplicationController
  ...
  def show 
    ...
    logger.info(params[:s_channel_id])
    ...
    @t_user_channels.each do |u_channel|
      if u_channel.userid != params[:user_id]
        ...
        logger.info("TGroupMessageID")
        logger.info(@t_group_message.id.to_s)
        temp_msgid += @t_group_message.id.to_s
        logger.info("TempMsgID")
        logger.info(temp_msgid)
...
def deletemsg
  ...
  def showthread         
    ...
    @t_group_thread.save
    Rails.logger.info('..........................')
    Rails.logger.info(@t_group_thread.id)
    ...
    @t_user_channels.each do |u_channel|
      Rails.logger.info('user id check........')
      Rails.logger.info(u_channel.userid)
      ...
      if u_channel.userid != params[:user_id]
        ...
        Rails.logger.info('temp_msgid>>>>>>>>>')
        Rails.logger.info(temp_msgid)
        u_channel.unread_thread_message = temp_msgid
        Rails.logger.info('u_channel.unread_channel_message >>>>>>>>')
        Rails.logger.info(u_channel.unread_thread_message)
